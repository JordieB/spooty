...
if st.button("Create Playlist with Set Frequency Range"):
    ...
    existing_playlist = next(
        (pl for pl in existing_playlists["items"] if pl["name"] == playlist_name), None
    )
    ...
    if playlist_created:
        ...
        st.success(
            f"Playlist created successfully! ID: {playlist_id} | Last track added at: {last_track_added_time}"
        )
    else:
        st.toast("Playlist updated!", icon="🔄")
        st.success(
            f"Playlist updated successfully! ID: {playlist_id} | Last track added at: {last_track_added_time}"
        )
